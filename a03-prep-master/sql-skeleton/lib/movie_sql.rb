...
def biggest_years_for_little_danny
  MovieDatabase.execute(<<-SQL)
    select movies.yr, count(movies.title) as count from movies
    join castings on castings.movie_id = movies.id
    join actors on actors.id = castings.actor_id
...
def count_bad_actors
  MovieDatabase.execute(<<-SQL)
    select count(actors.name) as num_bad_actors from actors
...
def chris_is_missed
  MovieDatabase.execute(<<-SQL)
    select m.title as title, a.name as name from (select movies.title as title, castings.movie_id as movie_id from movies
      join castings on castings.movie_id = movies.id
      join actors on actors.id = castings.actor_id
      where actors.name = 'Chris Farley') as m
      join (select actors.name as name, castings.movie_id as movie_id, castings.ord as ord from actors
      join castings on castings.actor_id = actors.id
      join movies on movies.id = castings.movie_id
